...
const props = defineProps({ visualStyles: Array })
...
function submit() { form.post('/projects') }
</script>
...
<template>
    ...
    <AppLayout>
        <div class="max-w-2xl">
            <Link href="/projects" class="inline-flex items-center gap-1.5 text-sm text-text-muted hover:text-text-primary mb-6 transition-colors">
                <ChevronLeft class="w-4 h-4" /> Proyectos
            </Link>
            <h1 class="text-xl font-semibold text-text-primary mb-6">Nuevo proyecto</h1>
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <Link href="/projects" class="text-sm text-text-muted hover:text-text-primary transition-colors">Cancelar</Link>
                </div>
            </form>
        </div>
    </AppLayout>
